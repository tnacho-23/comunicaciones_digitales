        ...
        for ifc = 1:length(fc_vals)
            ...
            for idx = 1:length(EbN0_dB)
                ...
                for run = 1:num_runs
                    ...
                    % Estimación CSI con pilotos y spline cúbico
                    rx_pilots = y(pilot_pos);
                    csi_est = rx_pilots / pilot_sym;

                    % Interpolación cúbica a posiciones de datos
                    csi_interp_full = interp1(pilot_pos, csi_est, data_pos, 'spline');
                    rx_data = y(data_pos);
                    csi_used = csi_interp_full;
                    ...
                    % Guardar constelaciones si corresponde
...
xlabel('E_b/N_0 [dB]');
ylabel('BER');
title('BER para QPSK en canal Multipath + AWGN (con interpolación cúbica spline)');
