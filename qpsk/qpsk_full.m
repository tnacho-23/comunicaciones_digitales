...
nBitPerSym = 52*2;       % Bits por símbolo OFDM (QPSK: 2 bits por símbolo)
...
v = (50)/3.6;            % Velocidad del móvil (50 km/h -> m/s)
fmax = v/lambda;         % Frecuencia Doppler máxima
nTap = 10;               % Número de taps del canal
N = 10;                  % Número de paths para el modelo de movilidad

% Prealocación para almacenar BER
simBer = zeros(1, length(EbN0dB));
theoryBerAWGN = 0.5*erfc(sqrt(10.^(EbN0dB/10)));  % BER teórico para BPSK
theoryBerAWGN_QPSK = 0.5*erfc(sqrt(10.^(EbN0dB/10)/sqrt(2))); % BER teórico para QPSK
theoryBerRayleigh_QPSK = 0.5*(1-sqrt(10.^(EbN0dB/10)./(10.^(EbN0dB/10)+2)));
...
for ii = 1:length(EbN0dB)
    ...
    ipBitReshaped = reshape(ipBit, 2, nBitPerSym*nSym/2).';
    ipMod = (1/sqrt(2))*(2*ipBitReshaped(:,1)-1 + 1j*(2*ipBitReshaped(:,2)-1));
    ipMod = reshape(ipMod, nBitPerSym/2, nSym).';
    
    % Mapeo a subportadoras
    xF = [zeros(nSym,6) ipMod(:,1:nBitPerSym/4) zeros(nSym,1) ...
          ipMod(:,nBitPerSym/4+1:nBitPerSym/2) zeros(nSym,5)];
    ...
    yMod = yF(:,[6+(1:nBitPerSym/4) 7+(nBitPerSym/4+1:nBitPerSym/2)]);
    
    % Demodulación QPSK
    ipModHat = yMod;
    ipBitHat = [real(ipModHat)>0 imag(ipModHat)>0];
...
% Gráficos
figure;
semilogy(EbN0dB, theoryBerAWGN_QPSK, 'b-', 'LineWidth', 2);
hold on;
semilogy(EbN0dB, theoryBerRayleigh_QPSK, 'r--', 'LineWidth', 2);
semilogy(EbN0dB, simBer, 'ks-', 'LineWidth', 2);
grid on;
axis([0 30 1e-5 1]);
xlabel('Eb/N0 (dB)');
ylabel('Bit Error Rate');
legend('AWGN Teórico QPSK', 'Rayleigh Teórico QPSK', 'Simulación (AWGN+Rayleigh+Movilidad) QPSK', 'Location', 'southwest');
title('BER para OFDM-QPSK con AWGN, Rayleigh y Movilidad');
