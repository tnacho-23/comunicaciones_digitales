...
for iN = 1:length(N_vals)
    ...
    legend_entries{end+1} = sprintf('Canal Rayleigh plano, N = %d', N);
    plot_idx = plot_idx + 1;
    ...
    xlabel('E_b/N_0 [dB]');
    ylabel('BER');
    title(sprintf('BER para QPSK con N = %d símbolos piloto (canal plano Rayleigh)', N));
    ...
    sgtitle(sprintf('Constelaciones con N = %d símbolos piloto (canal plano Rayleigh)', N));
end
