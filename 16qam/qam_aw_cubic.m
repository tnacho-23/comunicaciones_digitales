...
% Mapeo 16QAM normalizado
re = [-3 -1 3 1];
im = [3 1 -3 -1];
[Re, Im] = meshgrid(re, im);
mapping = (Re(:) + 1j * Im(:)) / sqrt(10); % Normalización energía promedio = 1
...
for iN = 1:length(N_vals)
    ...
    % Curva teórica (no válida para 16QAM pero ilustrativa)
    ...
    xlabel('E_b/N_0 [dB]');
    ylabel('BER');
    title(sprintf('BER para 16QAM con N = %d símbolos entre pilotos (Interpolación cúbica)', N+1));
