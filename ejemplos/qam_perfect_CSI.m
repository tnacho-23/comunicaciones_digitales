...
nFFT = 64;
nDSC = 52;
M = 16;                               % Orden de modulación (16QAM)
k = log2(M);                          % Bits por símbolo QAM
nBitPerSym = nDSC * k;               % Bits por símbolo OFDM
nSym = 1e4;
SNRdB = -2:1:30;
EbN0dB = SNRdB;
EsN0dB = EbN0dB + 10*log10(nDSC/nFFT) + 10*log10(64/80) + 10*log10(k); % Ajuste de energía
...
theoryBerAWGN = berawgn(EbN0dB, 'qam', M);
theoryBerRayleigh = berfading(EbN0dB, 'qam', M, 1);

for ii = 1:length(SNRdB)
    % Transmisor
    ipBit = randi([0 1], 1, nBitPerSym*nSym);
    ipBitReshaped = reshape(ipBit, k, []).';
    % Mapeo 16QAM Gray (normalizado a potencia unitaria)
    ipMod = qammod(bi2de(ipBitReshaped), M, 'gray', 'InputType', 'integer', 'UnitAveragePower', true);
    ...
    if ii == 1
        ...
        title('Constelación Transmitida (16QAM)');
    ...
    % Demodulación y BER
    yModVec = reshape(yMod.', [], 1);
    ipBitHat = de2bi(qamdemod(yModVec, M, 'gray', 'OutputType', 'integer'), k);
...
title('BER para OFDM-16QAM con canal móvil');
